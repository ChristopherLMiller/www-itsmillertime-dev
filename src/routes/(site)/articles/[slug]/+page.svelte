...
<Newspaper heading="From My Desk" subheading={post.title} headingTransitionName={`article-headline-${post.slug}`} columns={2} featuredImage={post.featuredImage} featuredImageTransitionName={`article-featured-image-${post.slug}`}>
  {#snippet meta()}
  <div class="meta">
    ...
    <span>First written {new Date(post.createdAt).toLocaleDateString('en-US', {
        year: 'numeric',
        month: 'long',
        day: 'numeric'
      })}
