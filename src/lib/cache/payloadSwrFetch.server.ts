...
import { createPayloadSwrFetch, DEFAULT_STALE, DEFAULT_TTL } from '$lib/cache/payloadSwrCore';
...
export function wrapFetchWithPayloadSwr(
	...
	const envStale = env.PAYLOAD_SWR_STALE_SECONDS
		...
		: undefined;
	const envTtl = env.PAYLOAD_SWR_TTL_SECONDS
		? Number.parseInt(env.PAYLOAD_SWR_TTL_SECONDS, 10)
		: undefined;

	const defaultStaleSeconds =
		explicitDefaults?.staleSeconds ?? (Number.isFinite(envStale) ? envStale! : DEFAULT_STALE);
	const defaultTtlSeconds =
		explicitDefaults?.softTtlSeconds ?? (Number.isFinite(envTtl) ? envTtl! : DEFAULT_TTL);
	...
	return createPayloadSwrFetch(innerFetch, baseURL, cacheManager, {
		defaultStaleSeconds,
		defaultTtlSeconds,
		isBackendAvailable: () => Boolean(env.UPSTASH_REDIS_REST_URL && env.UPSTASH_REDIS_REST_TOKEN),
