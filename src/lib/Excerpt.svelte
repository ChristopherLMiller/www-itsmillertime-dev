...
<article style:view-transition-name={`article-${item.slug}`}>
	{#if featuredImage}
		<Image
			transitionName={`article-featured-image-${item.slug}`}
			image={featuredImage}
			hasBorder={true}
		/>
	{/if}
	<a
		...
		style:view-transition-name={`article-headline-${item.slug}`}>{item.title}</a
	>
	<div class="meta">
		...
		{#if item.category}
			<div class="category" style:view-transition-name={`article-category-${item.slug}`}>
				Filed under: <a href="/articles?category={item?.category?.slug}">{item?.category?.title}</a>
			</div>
		{/if}
	</div>
	<p class="excerpt" style:view-transition-name={`article-content-${item.slug}`}>
